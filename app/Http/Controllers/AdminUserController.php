...
class AdminUserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('department') && !empty($request->department)) {
                $users->where('deparment_id', $request->department);
            }
            ...
            return DataTables::of($users)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<div class="btn-group m-1"><a href="' . route('adminusersprofile', $row->id) . '" class="view btn btn-primary btn-sm" data-bs-toggle="tooltip" title="View Profile" data-bs-placement="top" data-bs-original-title="View Profile"><i class="fa fa-eye"></i><a href="' . route('adminusers.edit', $row->id) . '" class="edit btn btn-primary btn-sm" data-bs-toggle="tooltip" title="Edit Profile" data-bs-placement="top" data-bs-original-title="Edit Profile"><i class="fa fa-edit"></i></a>';
        ...
        return view('adminusers.index', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        $specificRoles = "";
        $specificRoles = Role::whereIn('name', ['Admin', 'Manager', 'Team Leader', 'Service Team Leader', 'Engineer', 'Sales Team Leader', 'Sales Person'])->get();
        $data['specificRoles'] = $specificRoles;
        return view('adminusers.create', $data);
    }

    public function store(Request $request)
    {
        
        //dd($request->deparment_id);
        $data['title'] = 'Create User';
        ...
        if(isset($request->shift_id) && !empty($request->shift_id)) {
            ...
            if(isset($shift)) {
                $data['duty_start'] = $shift->shift_start;
                $data['duty_end'] = $shift->shift_end;
            }
        }
        ...
        if (isset($request->main_machine_type) && count($request->main_machine_type) > 0) {
            $iuserId = $user->id;
        ...
        if(isset($request->is_saleuser) && $request->is_saleuser == 1)
        {
            ...
            toastr()->success('User created successfully');
            return redirect()->route('adminusers.index', ['main_machine_type' => $request->user_main_machine_type])->with('success', 'User created successfully');
        }
    }
    ...
    public function edit(User $user)
    {
        ...
        return view('adminusers.edit', $data);
    }
    ...
    public function update(Request $request, User $user)
    {
        ...
        if(isset($request->is_saleuser) && $request->is_saleuser == 1)
        {
            ...
            toastr()->success('User updated successfully');
            return redirect()->route('adminusers.index', ['main_machine_type' => $request->user_main_machine_type])->with('success', 'User updated successfully');
        }
    }

    public function adminusersProfile($id, Request $request) 
    {    
        ...
        if($userDetail){
            return view('adminusers.userprofile', $data);
        } else {
            return view('adminusers.userprofile', $data);
        }
    }
