    ...
    public function index(Request $request)
    {
        if ($request->is_past_record == '1') {
            $data = Todo::join('todo_assign_users as TAS', 'todos.id', '=', 'TAS.todo_id')
                        ...
                        ->whereDate('todos.assign_date_time', '<', date('Y-m-d'))
                        ->with([
                            'todoUser', 
                            'todoTasks', 
                            'priority', 
            ...
            foreach ($request->assign_users as $userKey => $user) {
                if ($user != "") {
                    ...
                    if (!empty($user->device_token)) {
                        ...
                        $title = 'New Todo';
                        $body = 'You have been assign a new todo. '. $data['title'];
    ...
    public function todosFilter(Request $request)
    {
        ...
        $data = Todo::join('todo_assign_users as TAS', 'todos.id', '=', 'TAS.todo_id')
                        ->where(function ($query) use($user_id) {
                            $query->where('todos.user_id',$user_id)
                                ->orWhere('TAS.assign_user_id', $user_id);
                        })
            ...
            ->when($status !== null && $status !== '' && ($status == 2 || $status == 3), function ($query) use ($status) {
        ...
        if (!empty($data['todo']) || !empty($data['todoAssignUser'])) {
            return response()->json([
                'success' => true,
                'message' => 'Todo fetched successfully',
                'data' => $data
