...
class SalesPersonController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($salesPerson)) {
            ...
            //     //$salesPerson = SalesPerson::with('salseUserDetail','saleAssignUserDetail')->where('id', $lastInsertId)->first();
            //     $user = User::find($request->sale_assign_user_id);
            //     if (!empty($user->device_token)) {
            //         $title = 'Lead assigned successfully';
            //         $body = $user->name . ' assigned new Lead, lead no:' . $salesPerson->id;
        ...
        $salesPerson->closed_date = !empty($request->closed_date) ? date('y-m-d', strtotime($request->closed_date)) : null;
         ...
         if (isset($data['image'])) {
            ...
            if (isset($data['video'])) {
                ...
                $filename = time() . '.' . $file->getClientOriginalExtension();
                $file->move(public_path('complaint/videos'), $filename);
                $salesPerson->video = $filename;
            }
            if (isset($data['audio'])) {
                $file = $data['audio'];
                $filename = time() . '.' . $file->getClientOriginalExtension();
                $file->move(public_path('complaint/audios'), $filename);
                $salesPerson->audio = $filename;
            }
    ...
    public function assignSalsePersonPriorityFilter(Request $request)
    {
        ...
        // if(isset($request->sale_assign_user_id) && $request->sale_assign_user_id!='')
        // {
        //     $sale_assign_user_id = $request->sale_assign_user_id;
        // }
        // else {
        //     $sale_assign_user_id = Auth::user()->id;
        // }
        $sale_assign_user_id = $request->sale_assign_user_id;
        $date = $request->date;
        ...
        //$salse = SalesPerson::where('sale_user_id', Auth::user()->id)
            //->orWhere('sale_assign_user_id', Auth::user()->id)
            ->where(function ($query) use ($sale_assign_user_id, $date, $status_id, $closed_date, $favorite, $party_name, $main_machine_type) {
                if ($sale_assign_user_id) {
                    // in filter for admin
                    $query->where('main_machine_type', $main_machine_type)
                          ->where('sale_assign_user_id', $sale_assign_user_id);
                }
                else {
                    // for Default list for admin
                    $luserid = Auth::user()->id;
                    if($luserid!='' && ($luserid=='191')){
                        $query->where('main_machine_type', $main_machine_type);
                              //->where('sale_user_id', Auth::user()->id);
                    } else if($luserid!='' && ($luserid=='117'))
                    {
                        // Default list for Manager / or Sales Leader
                        $query->where('main_machine_type', $main_machine_type)
                              ->where('sale_user_id', Auth::user()->id);
                    } else{
                        // Default list for Engineers
                    $query->where('sale_assign_user_id', Auth::user()->id);
                    }
                }
                if ($date) {
                ...
                if (isset($party_name) && $party_name!='') {
                  //dd('in iffff');  
                    $query->where('partyname', 'LIKE', "%$party_name%");
                }
            })
